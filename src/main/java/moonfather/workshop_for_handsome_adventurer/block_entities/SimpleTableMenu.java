...
public class SimpleTableMenu extends AbstractContainerMenu
{
    ...
    {
        return OptionsHolder.COMMON.SimpleTableNumberOfSlots.get();
    }
    ...
    public void renameChest(String newName)
    {
        ...
        else if (this.inventoryAccessHelper.chosenContainerItem != null)
        {
            ...
            this.inventoryAccessHelper.chosenContainerItem.setHoverName(Component.literal(newName));
            player.giveExperienceLevels(-1);
        }
    }
    ...
    private void initExcessSlotMapInternal(int slot, int offset)
    {
        ...
        {
            this.DataSlots.setSlotValue(slot, 0);
            this.synchronizeDataSlotToRemote(slot, 0);
        }
    }
    ...
    private static class DisabledContainer extends SimpleContainer
    {
        ...
        {
            return this.disabled ? MARKER_FOR_DISABLED : super.getMaxStackSize(); // put 999 when SD is fixed
        }
    ...
    public static class VariableSizeContainerWrapper extends SimpleContainer implements IExcessSlotManager
    {
        ...
        {
            return slot < internal.getContainerSize() ? internal.getItem(slot) : ItemStack.EMPTY;
        }

        @Override
        public ItemStack removeItem(int slot, int count)
        {
            return slot < internal.getContainerSize() ? internal.removeItem(slot, count) : ItemStack.EMPTY;
        }

        @Override
        public ItemStack removeItemNoUpdate(int slot)
        {
            return slot < internal.getContainerSize() ? internal.removeItemNoUpdate(slot) : ItemStack.EMPTY;
        }

        @Override
        public void setItem(int slot, ItemStack itemStack)
        {
            if (slot < internal.getContainerSize())
            {
    ...
    public static class VariableSizeItemStackHandlerWrapper extends SimpleContainer
    {
        ...
        public boolean canPlaceItem(int slot, ItemStack itemStack)
        {
            return slot < internal.getSlots() && internal.isItemValid(slot, itemStack);
        }

        @Override
        public ItemStack getItem(int slot)
        {
            return slot < internal.getSlots() ? internal.getStackInSlot(slot) : ItemStack.EMPTY;
        }

        @Override
        public ItemStack removeItem(int slot, int count)
        {
            return slot < internal.getSlots() ? internal.extractItem(slot, count, false) : ItemStack.EMPTY;
        }

        @Override
        public ItemStack removeItemNoUpdate(int slot)
        {
            return slot < internal.getSlots() ? internal.extractItem(slot, 9999, false) : ItemStack.EMPTY;
        }
        ...
        public void setItem(int slot, ItemStack itemStack)
        {
            if (slot < internal.getSlots())
            {
				...
				{
					internal.insertItem(slot, itemStack, false);
				}
            }
        }
        ...
        public int getMaxStackSize()
        {
            return internal.getSlots() > 1 ? internal.getSlotLimit(1) : (internal.getSlots() > 0 ? internal.getSlotLimit(0) : 64); // this is not how we nice girls write code.
        }
    }
    /////////////////////////////////////////////////////////////////////////
